...
public class State implements IState, IRouting {
	...
	public State(NodeInfo info) {
		...
		this.finger = new NodeInfo[NFINGERS];
	...
	public void setBlankEnventSource(EventSource s) {
		BLANK_ENVENT_SOURCE = s;
	}
	...
	public synchronized NodeInfo closestPrecedingFinger(int id) {
		...
		for(int i = 0; i < finger.length - 1; i++) {
			...
			if(DHTBase.inInterval(id, (info.id + (int)Math.pow(2, i) )%IRouting.NKEYS , (info.id + (int)Math.pow(2, i + 1))%IRouting.NKEYS, true)) {
				return  finger[i];
			}
		}
