...
//		routing.joinCheck();
	...
	protected NodeInfo findPredecessor(int id) throws Failed {
		...
		if (info.id != succ.id) {
			while (!inInterval(id, info.id, succ.id)) {
				 ...
				 */
				if(pred != null && inInterval(id, pred.id, info.id)) {
					return pred;
				}
				
				if(getNodeInfo().equals(info) && pred != null) {
					return pred;
				}
				if(getNodeInfo().equals(info) && pred == null) {
					return info;
	...
	protected TableRep transferBindings(int predId) {
		...
		/*
		 * when dropping down the bindings, we should to keep the listeners in the date
		 */
		this.state.fixListener(predId);
		return db;
	}
	...
	public void fixAllListeners() throws Error, Failed{
		this.state.fixListener(-1);
	}
	...
	public void stopListening(int id, String key) {
		...
		this.state.removeListener(id, key);
	}
	
	private EventListener changeBindingListener() {
		return new EventListener() {
	        public void onEvent(InboundEvent ev) {
	            try {
	            	String key = ev.getData(String.class);
	            	// stop local listening under key
	            	state.removeCallback(key);
	            	
	            	EventListener changeBindingLister = changeBindingListener();
	            	EventListener additionBindingListener = additionBindingListener(key);
	            	// renew the listen on procedule
	            	int id = DHTBase.NodeKey(key);
	            	
	            	//find the server node for the key;
	        		try {
						 NodeInfo serverNode = findSuccessor(id);
						
						//force client to register eventSource and return it into server, current node
						 EventSource eventSource = client.listenForBindings(serverNode, info.id, key);
						 eventSource.register(additionBindingListener, IDHTNode.NEW_BINDING_EVENT);
						 eventSource.register(changeBindingLister, IDHTNode.CHANGE_BINDING_EVENT);
						 //add it into state and store its status
						 state.addCallback(key, eventSource, serverNode);
						 // start listening
						 eventSource.open();
					} catch (Failed e) {
						e.printStackTrace();
	                ...
	                severe("Error while deserializing event data.");
	            }
	        }
	    };
	}
	
	private EventListener additionBindingListener(final String key) {
		return new EventListener() {
	        public void onEvent(InboundEvent ev) {
	            try {
	            	System.out.println(String.format("** Binding event (%s): %s -> %s", ev.getName(), key, ev.getData(String.class)));
	            } catch (IOException e) {
	            	System.out.println("Error while deserializing event data.");
	            }
	        }
	    };
	}
	...
	public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
		 ...
		 */
		EventListener changeBindingListener = changeBindingListener();
		
		//id
		int id = DHTBase.NodeKey(key);
		//find the server node for the key;
		NodeInfo serverNode = this.findSuccessor(id);
		
		//stop event generation in the server
		this.client.listenOff(serverNode, this.info.id, key);
		//force client to register eventSource and return it into server, current node
		 EventSource eventSource = this.client.listenForBindings(serverNode, this.info.id, key);
		 eventSource.register(listener, IDHTNode.NEW_BINDING_EVENT);
		 eventSource.register(changeBindingListener, IDHTNode.CHANGE_BINDING_EVENT);
		 //add it into state and store its status
		 this.state.addCallback(key, eventSource, serverNode);
	...
	public void listenOff(String key) throws DHTBase.Failed {
		...
		//stop local listen callback
		this.state.removeCallback(key);
